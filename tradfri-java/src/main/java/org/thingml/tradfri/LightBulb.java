...
public class LightBulb {
	...
	public void setIntensity( int intensity, int transitionTime ) {
		try {
			JSONObject json = new JSONObject();
			JSONObject settings = new JSONObject();
			JSONArray array = new JSONArray();
			...
			settings.put( TradfriConstants.TRANSITION_TIME, transitionTime );
			String payload = json.toString();
			gateway.set( TradfriConstants.DEVICES + "/" + this.getId(), payload );
	...
	protected void parseResponse( CoapResponse response ) {
		...
		if( updateListeners ) {
			for( TradfriBulbListener l : listners ) {
